...
# SGHMC based on Wenzel parameterization with symplectyc Euler discretization
# See course notes for the precise equations
# p_{t+1} = p_t - h * grad U - h * friction * p_t + sqrt(2*friction * mass * temperature * h) * W_t
# theta_{t+1} = theta_t + h * p_{t+1} / mass,
# where mass is fixed to 1.
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for param in group["params"]:
                ...
                if len(state) == 0:
                    state["step"] = 0
                    state["momentum"] = torch.randn(param.size(), dtype=param.dtype).to(
                        param.device
                    )

                n, T = group["num_data"], group["temperature"]
                lr, wd = group["lr"], group["weight_decay"]
                h = np.sqrt(lr / n)
                friction = (1 - group["momentum_decay"]) / h
